...
function dashboard() {
  ...
  if (document.getElementById('dashboard') || document.getElementById('plan-detail')) {
    ...
    const urls = [
      activeUrl + `/itinerary/${user.user.email}/user`,
              ...
              itinerary.map((item, i) => {
                ...
                eachRecord = `
                          ...
                          <td>${item.drive_option}</td>
                           <td>${item.no_hours}</td>
                            <td><span class="label label-table ${className}">${
                  item.status
                }</span></td>
                     </tr>`;
              ...
              if (planClicked) {
                           ...
                           <a href="#"><div class="col-sm-6  col-md-6 col-lg-4" >
                                <div class="widget-bg-color-icon card-box">
                                    <div class="bg-icon bg-icon-info pull-left" >
                                        <img src="${item.image ||
                                          item.images}" style="height:90px"/>
                                    </div>
                                    <div class="col-lg-6 pull-right text-right">
                                        <h3 class="text-dark"><b className="">${item.car_type} ${
                    item.model
                  } ${item.car_year}</b></h3>
                ...
                selectedItineraries.map((item, i) => {
                  if (item.status == 'Paid') {
                    ...
                    payButton = `<td class="">
                                    <a  href="#" onclick="setClickedItinerary(this)" data-driver="${
                                      planClicked.cars_on_plan[0].assigned_driver_name
                                    }" data-driver_email="${
                      planClicked.cars_on_plan[0].assigned_driver_email
                    }" data-driver-plate_num="${
                      planClicked.cars_on_plan[0].assigned_driver_phone
                    }" data-driver_loc="${
                      planClicked.cars_on_plan[0].assigned_driver_location
                    }" data-status="${item.status}"  data-id="${item.id}" data-travel_option="${
                      item.travel_option
                    }" data-pickup_time=${item.pickup_time} data-start_time="${formatDate(
                      new Date(item.start_time),
                    )}" data-end_time="${item.end_time}"  data-start_location="${
                      item.start_location
                    }" data-destination="${item.destination}" data-drive_option="${
                      item.drive_option
                    }"  class="table-action-btn btn-custom btn-purple right-bar-toggle "><i class="md md-chevron-right"></i></a>
                                </td>`;
                    ...
                    payButton = `<td class="">
                                    <a id="click-me"  href="#" onclick="setClickedItinerary(this)" data-driver="${
                                      planClicked.cars_on_plan[0].assigned_driver_name
                                    }" data-driver_email="${
                      planClicked.cars_on_plan[0].assigned_driver_email
                    }" data-driver-plate_num="${
                ...
                [planClicked].map((item, i) => {
                  ...
                  template2 = `<tr>
                                        <div class=""><strong>Plan ID : <a href="plan-detail">${
                                          item.plan_id
                                        }</a></strong></div><br/>
                                            <div class=""><strong>Plan :   ${
                                              item.plan_name
                                            } </div><br/>

                                            <div class=""><strong>Plan Category : ${
                                              item.plan_category_name
                                            } </strong></div><br/>
                                            <div class=""><strong>Created Date : ${formatDate(
                                              new Date(item.created_at),
                                            )} </strong></div><br/>
                                            ${actionBtn}

                                            <div style="display:none" class=""><strong>Duration: ${
                                              item.duration
                                            } hrs</strong></div><br/>
  ...
  switch (error.code) {
    case 1:
      document.querySelector('#error').innerHTML +=
        ":<br>You blocked the request <br> or <br> You need to add 'https://' to this pen's URL";
      break;
    case 2:
      document.querySelector('#error').innerHTML +=
        ':<br> The device has failed in getting your position';
      break;
    case 3:
      document.querySelector('#error').innerHTML +=
        ':<br> You need to increase the timeout value in positionOptions';
      break;
  }
}
...
//for admin
function driversNearBy(MAP, startLocationCordinates) {
  //lat , long
  const user = JSON.parse(localStorage.getItem('userToken'));
...
function showUserDrift(startlocAdress, destinationAddress, geocoder, resultsMap) {
  ...
  geocoder.geocode({ address: address1 }, function(results, status) {
    if (status === 'OK') {
      ...
      var contentString =
        '<div id="content"><h1>Pickup Location.' + `</h1><p>${address1}</p></div>`;
  ...
  geocoder.geocode({ address: address2 }, function(results, status) {
    if (status === 'OK') {
      ...
      var contentString = '<div id="content"><h1>Destination.' + `</h1><p>${address2}</p></div>`;
...
function calTravelDist(startlocAdd, endlocAdd, maps) {
  ...
  helper.getDistanceDrivingMiles(originText, destText, function(obj) {
    ...
    var notificationx = alertify.notify(builder.join(''), 'successw', 35, function() {
      console.log('dismissed');
    });
  });
}
